...
旧多跳编排器的远端入站没有数据库主记录，无法可靠恢复路线、所有权或授权关系，因此不做隐式导入。旧页面、写接口和 MCP 入口均已移除，避免两套编排器同时修改 Xray。

退役规则：

1. 部署前只读盘点旧 Tag、所有权记录和快照；盘点为零时不执行数据写入。
2. 发现旧资源时必须由管理员逐项确认目标和所有权后清理，不能自动授权或转换为用户转发。
3. 现行转发系统只使用数据库主记录和 `rd-tun-*` 资源身份，由 reconciler 与 expiry Guard 管理完整生命周期。
4. 节点任意门继续走单服务器受管节点事务，删除普通节点时同步清理对应入站，不再借用旧多跳编排器。
...
5. 现行 reconciler 为每个 hop 的 Add/Remove 携带稳定 `mutation_id`。升级后的 Agent 将删除栅栏持久化到侧车文件，防止响应超时后迟到的 Add 在已确认删除后重新监听端口；旧 Agent 兼容忽略该字段，但不具备该保护，因此也应升级或重装。
