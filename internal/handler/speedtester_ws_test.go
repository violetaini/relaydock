...
func TestSpeedtesterUpdateMessageOnlyTargetsCompatibleOlderClients(t *testing.T) {
	...
	if update.Type != "update" || update.Version != "0.6.8" {
	...
	for _, testCase := range []struct {
		...
		{name: "already current", version: "0.6.8", caps: []string{speedtesterSelfUpdateCapability}, goos: "linux", goarch: "amd64"},
		{name: "newer client", version: "0.6.9", caps: []string{speedtesterSelfUpdateCapability}, goos: "linux", goarch: "amd64"},
